...
def get_nakit_kasa_sube_ozet(userid: int, musterino: int, yil: int) -> list[dict]:
    ...
    try:
        for yil_filter in [str(yil), None]:
            params_yil = (musterino, yil_filter) if yil_filter else (musterino,)
            yil_where  = f"AND {_yil_col} = ?" if yil_filter else ""
            rows = conn.execute(f"""
                SELECT
    ...
    try:
        for yil_filter in [str(yil), None]:
            ...
            if sube_id is not None:
                params = (musterino, sube_id, yil_filter) if yil_filter else (musterino, sube_id)
                rows = conn.execute(f"""
                    SELECT
                ...
                params = (musterino, yil_filter) if yil_filter else (musterino,)
                rows = conn.execute(f"""
                    SELECT
                        h.id, h.tarih, h.aciklama,
                        COALESCE(s.{_sube_ack}, '(Şubesiz)') AS sube_adi,

...
class DetayDialog(QDialog):
    ...
    def __init__(self, baslik: str, ozet_rows: list[dict],
                 ...
                 yil: int | None = None,        # Dashboard'dan seçili yıl
                 parent=None):
        super().__init__(parent)
        ...
        self._default_yil = yil  # Dashboard'dönemi kaydet
        ...
        # Dashboard'dan gelen yıl varsa onu kullan, yoksa sistem yılını kullan
        default_yil = yil if yil is not None else current_year
        ...
        # Seçili yılın listede bulunması için aralığı genişlet
        min_yil = min(current_year - 5, default_yil - 1)
        max_yil = max(current_year + 1, default_yil + 1)
        for y in range(max_yil, min_yil, -1):
            self.combo_yil.addItem(str(y), int(y))
        # Dashboard'dan gelen (veya mevcut) yılı varsayılan seç
        idx = self.combo_yil.findData(default_yil)
        if idx >= 0:
        ...
        if self._direct_detay:
            ...
            # Yükleme sırasında filtrelerin tetiklenmemesi için sinyalleri kes
            ...
            try:
                ...
                print(f"[DetayDialog] Veri yüklenemedi: {e}")
                rows = []

            # Gelir / Gider / Net özet hesapla
            # genel_hesap tipinde: ayrı "gelir" ve "gider" kolonları
            # hareketler tipinde: "gelirgider" + "tutar" kolonu
            toplam_gelir = 0.0
            toplam_gider = 0.0
            for r in rows:
                # genel_hesap tipi: gelir ve gider ayrı kolonlarda
                g_val = r.get("gelir")
                d_val = r.get("gider")
                if g_val is not None or d_val is not None:
                    try:
                        toplam_gelir += float(g_val or 0)
                    except Exception:
                        pass
                    try:
                        toplam_gider += float(d_val or 0)
                    except Exception:
                        pass
                else:
                    # hareketler tipi: gelirgider + tutar
                    gg = str(r.get("gelirgider") or r.get("gelirGider") or "").lower()
                    try:
                        tutar = float(r.get("tutar") or r.get("alinan_tutar1") or 0)
                    except Exception:
                        tutar = 0.0
                    if gg == "gelir":
                        toplam_gelir += tutar
            ...
            self._detay_gelir_lbl.setText(f"🟢 Gelir: {fmt_para(toplam_gelir)}")
            self._detay_gider_lbl.setText(f"🔴 Gider: {fmt_para(toplam_gider)}")
            if net >= 0:
            ...
            # Sinyalleri geri aç (artık kullanıcı etkileşimine hazır)
    ...
    def _on_kart_clicked(self, row: dict):
        ...
        # Ay/Yıl filtrelerini sebe döneme sıfırla
        self.combo_ay.blockSignals(True)
        self.combo_ay.setCurrentIndex(0)   # Tüm Aylar
        self.combo_ay.blockSignals(False)

        # Yılı Dashboard dönemine göre ayarla (varsa)
        self.combo_yil.blockSignals(True)
        if self._default_yil is not None:
            ...
            self.combo_yil.setCurrentIndex(0)   # Tüm Yıllar
        self.combo_yil.blockSignals(False)
        ...
        for row_idx in range(self._tablo.rowCount()):
            ...
            if (selected_ay is not None) or (selected_yil is not None):
                ...
                if tarih_item:
                    ...
                    # Parse başarısızsa satırı GİZLEME (görünür bırak)
                    if ay is None and yil is None:
                        pass  # tarih parse edilemedi, filtre uygulama
                    else:
                        if selected_ay is not None and ay != selected_ay:
                            match = False
                        if selected_yil is not None and yil != selected_yil:
                            match = False
                # tarih hücresi yoksa satırı yine de göster (gizleme)
